...
import { BillingPlan, getPayPalPlanUrl } from "@/lib/payments";
...
export function PricingSection() {
  ...
  const handleCheckout = (plan: BillingPlan) => {
    const url = getPayPalPlanUrl(plan);
    if (url) {
    ...
    <section id="pricing" className="py-24 border-t border-border/30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {plans.map((plan) => {
            const hasPayPalLink = Boolean(getPayPalPlanUrl(plan.id));
            ...
            return (
              <div
                ...
                className={`relative rounded-2xl p-8 flex flex-col transition-all ${
                  plan.highlighted
                    ? "gradient-card glow-border shadow-glow scale-100 md:scale-105"
              ...
              >
                ...
                <div className="mb-6">
                  ...
                  <div className="mt-2 text-sm font-medium text-primary">{t(`${plan.leadsKey}.f1`)}</div>
                </div>
                ...
                >
                  {hasPayPalLink ? t("pricing.paypal") : t(plan.ctaKey)}
                  {hasPayPalLink ? <ExternalLink className="ml-2 h-4 w-4" /> : null}
                </Button>
              </div>
            );
          })}
        ...
        <p className="text-center text-sm text-muted-foreground mt-8">{t("pricing.footer")}</p>
      </div>
    </section>
  );
}
